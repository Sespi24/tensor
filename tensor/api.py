...
#JSON
df = pd.DataFrame(data=data, copy=True, index=columns).transpose()
df['IND_DAY'] = pd.to_datetime(df['IND_DAY'])
df.to_json('data-index.json', date_format='iso', date_unit='s')
s = df.to_json()
print(s)
...
#CSV
df = pd.DataFrame(data=data, copy=True, index=columns).transpose()
df.to_csv('data.csv', na_rep='(missing)', date_format='%B %d, %Y')
